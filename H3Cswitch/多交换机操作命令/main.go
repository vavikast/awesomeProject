...
func main() {
	...
	// 输入要进行命令的交换机
	var a string
	fmt.Println("请输入要操作的交换机，以逗号方式输入，例如1,2。 核心交换机代表1,办公交换A1表示11，办公交换A1表示12,办公交换机B表示13,办公交换机C表示14,服务器交换机表示16，AC表示17。快捷操作必备: 交换机添加mac地址输入17,AC添加mac输入18,AC删除mac输入19")
	_, err := fmt.Scanln(&a)
	if err != nil {
		log.Fatal("输入错误:", err)
	}
...
func Addnum(num int) error {
	if num == 1 {
		HostPasswods["192.168.102.1:22"] = password01
	} else if num == 11 {
		HostPasswods["192.168.102.11:22"] = password11
	} else if num == 12 {
		HostPasswods["192.168.102.12:22"] = password12
	} else if num == 13 {
		HostPasswods["192.168.102.13:22"] = password13
	} else if num == 14 {
		HostPasswods["192.168.102.14:22"] = password14
	} else if num == 16 {
		HostPasswods["192.168.102.16:22"] = password16
	} else if num == 17 {
		HostPasswods["192.168.102.11:22"] = password11
		HostPasswods["192.168.102.12:22"] = password12
		HostPasswods["192.168.102.14:22"] = password14
		...
		HostPasswods["192.168.102.17:22"] = password17
		AcTurnOn = true
	} else if num == 19 {
		HostPasswods["192.168.102.17:22"] = password17
		AcTurnOff = true
